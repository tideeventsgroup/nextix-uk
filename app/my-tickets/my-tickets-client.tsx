...
import type { WalletTicket } from "../../lib/data";

export function MyTicketsClient({ upcoming, past }: { upcoming: WalletTicket[]; past: WalletTicket[] }) {
  ...
  return (
    <main className="inner-page ticket-wallet" id="top">
      <section className="wallet-heading">
        ...
        <nav>
          <button className={tab === "upcoming" ? "active" : ""} onClick={() => selectTab("upcoming")} aria-pressed={tab === "upcoming"}>Upcoming <span>{upcoming.length}</span></button>
          <button className={tab === "past" ? "active" : ""} onClick={() => selectTab("past")} aria-pressed={tab === "past"}>Past <span>{past.length}</span></button>
        </nav>
      </section>
      ...
      {list.length === 0 ? (
        ...
        <section className="wallet-layout">
          <aside>
            {list.map((t, i) => (
              <button className={active === i ? "active" : ""} onClick={() => { setActive(i); setAction(""); }} key={`${t.ref}-${t.slug}-${i}`}>
                <img src={t.image} alt="" />
